...
for node in nodes:
	...
	AverageEnergy += total_energy_mJ
	count += 1
	print("Node {}: {:.2f} mC ({:.3f} mAh) charge consumption, {:.2f} mJ energy consumption in {:.2f} seconds".format(node, total_charge_mC, total_charge_mC / 3600.0, total_energy_mJ, period_seconds))

print(AverageEnergy / count)
